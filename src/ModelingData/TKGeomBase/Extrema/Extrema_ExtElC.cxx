...
bool Extrema_ExtElC::PerpendicularCirclesExtrema(const gp_Circ& C1,
                                                 const gp_Circ& C2,
                                                 const gp_Pln&  aPlc1,
                                                 const gp_Pln&  aPlc2,
                                                 const double   aTolD,
                                                 const double   angPlanes)
{
  if (angPlanes <= M_PI_4)
  {
    return false; // The planes are more "parallel" than "perpendicular"
  }

  double aTolD2 = aTolD * aTolD;
  gp_Pnt aPc1   = C1.Location();
  gp_Pnt aPc2   = C2.Location();
  double aD1    = aPlc2.SquareDistance(aPc1);
  if (aD1 < aTolD2)
  {
    ...
    double aDist = aPc1.Distance(aPc2);
    double aMinR = std::min(C1.Radius(), C2.Radius());
    double aMaxR = std::max(C1.Radius(), C2.Radius());
    ...
    if (std::abs(aDist - aMaxR) < aTolD)
    {
      ...
      myPoint[0][0].SetValues(ElCLib::Parameter(C1, aP1), aP1);
      myPoint[0][1].SetValues(ElCLib::Parameter(C2, aP2), aP2);
      myIsPar = true; // indicator for infinite solutions
      return true;
    }
    ...
    // std::cout << "====> Extrema_ExtElC: unsupported case for circles in different planes." <<
    // std::endl; std::cout << "  Distance between centers: " << aDist << std::endl; std::cout << "
    // Angle between planes ...: " << angPlanes * 180.0 / M_PI << " deg" << std::endl; std::cout <<
    // "  Radius Circle 1 ........: " << C1.Radius() << std::endl; std::cout << "  Radius Circle 2
    // ........: " << C2.Radius() << std::endl;
  }
  return false;
}
...
Extrema_ExtElC::Extrema_ExtElC(const gp_Circ& C1, const gp_Circ& C2)
{
  ...
  if (!bIsSamePlane)
  {
    ...
    const double angPlanes = aDc1.Angle(aDc2);
